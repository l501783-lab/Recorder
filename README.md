...
- **匯入現有音檔**：手機錄音、會議錄音、mp3／m4a／wav／webm 都可以拖進來，用 Whisper 產生逐字稿。超過 25MB 的檔案會自動在本機解碼成 16kHz 單聲道、切成 8 分鐘一段送出，時間戳記會接續對齊，所以幾小時的訪談也處理得了（匯入的檔案沒辦法用瀏覽器即時辨識，一定要走 Whisper）
